...
# Javaria Ghafoor, rotating T-handle in zero-g, intermediate axis theorem, 27/04/2019
...
filler = cylinder(frame=tbar, pos=(-1, 0, 0), axis=(.5, 0, 0), radius=.75,
                  mass=0)  # fills space between two cylinders (is not involved in calculations)
...
Ixx = (1 / 2) * (cross.mass) * (cross.radius ** 2) + (cross.mass) * q ** 2 + (perp.mass / 4) * (perp.radius ** 2) + (
            perp.mass / 12) * (perp.length ** 2) + perp.mass * (perp.length / 2 + cross.radius - q)
Iyy = (1 / 4) * (cross.mass) * (cross.radius ** 2) + (1 / 12) * (cross.mass) * (cross.length ** 2) + cross.mass * (
            q ** 2) + (1 / 4) * perp.mass * (perp.radius ** 2) + (1 / 12) * (perp.mass) * (
                  perp.length ** 2) + perp.mass * (perp.length / 2 + cross.radius - q)
Izz = (1 / 2) * (perp.mass) * (perp.radius ** 2) + (1 / 4) * (cross.mass) * (cross.radius ** 2) + (1 / 12) * (
    cross.mass) * (cross.length ** 2)
...
xvec = arrow(pos=(10, 0, 0), length=(Ixx * tbar.omega.x) / 5, axis=basis_vectors[0], shaftwidth=.1, color=color.red,
             opacity=.7)
yvec = arrow(pos=(10 + xvec.length, 0, 0), length=(2 * Ixx * tbar.omega.y) / 5, axis=basis_vectors[1], shaftwidth=.1,
             color=color.green, opacity=.7)
zvec = arrow(pos=(10 + xvec.length, yvec.length, 0), length=(Ixx * tbar.omega.z) / 5, axis=basis_vectors[2],
...
totvec = arrow(pos=(10, 0, 0),
               ...
               axis=norm((xvec.length, yvec.length, zvec.length)), shaftwidth=.2, color=color.yellow)
